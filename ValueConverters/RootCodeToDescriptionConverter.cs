...
namespace SACEology
{
    ...
    /// </summary>
    public class RootCodeToDescriptionConverter : BaseValueConverter<RootCodeToDescriptionConverter>
    {
        public override object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            // If this badge's code exists in the performance standard database, return its description
            foreach (List<string> standard in PerformanceStandardDatabase)
            {
                if (code == standard[(int)PSProp.Code])
                {
                    return standard[(int)PSProp.Description];
                }
            }
            ...
            // If this badge's code exists in the assessment type database, return its description
            foreach (List<string> type in AssessmentTypeDatabase)
            {
                if (code == type[(int)ATProp.Code])
                {
                    return type[(int)ATProp.Description];
                }
            }
